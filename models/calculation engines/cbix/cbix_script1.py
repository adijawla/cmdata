...
all_names = [
    ...
    "Master_date_cell",
    "Target_CBIX_Price",
    "freights_perwmt_tradedata",
    "cbix_coefficients_determination_inputs",
...
def final_touch(df, name):
    data = df.copy()
    ex = ["MRN_Juruti_max_cargo", "Target_CBIX_Price"]
...
def restruct():
    ...
    for sh in all_names:
        ...
        if  "model_id" in data.columns:
            data = data.loc[data["model_id"] == "model_8"]
        ...
        exempts = ["Port_Linkages", "Canal", "global_factors", "Canals_Class", "vessel_class"]
