...
def weird_space(s):
    # 65289
    return "".join([a for a in s if ord(a) != 65289])
...
class upload():
    def __init__(self,snapshot_output_data):
        ...
        #labels = labels.exten
        uni_lable = list(set([weird_space(str(a).title()) for a in uni_lable]))
        db_label = list(set([weird_space(str(a).title()) for a in db_label]))
