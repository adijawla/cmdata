...
from retructure import priceForecastRestructure
...
class PriceForecastModel:
    ...
    def __init__(self):
        ...
        self.restructure = priceForecastRestructure()



        # self.dg114 = pd.read_csv("Inputs/EM_Supply with Countries.csv")

        self.dg114 = self.restructure.EM_SupplyCountries_Restructure()



        # self.projects2 = pd.read_csv("Inputs/EM_Sheet1.csv")

        self.projects2 = pd.DataFrame(self.dg114["Name"].copy())



        # self.projects = pd.read_csv("Inputs/EM_Project.csv")

        self.projects = pd.DataFrame(self.dg114["Name"].copy())



        # self.projects1 = pd.read_csv("Inputs/EM_Rank.csv")

        self.projects1 = self.restructure.EM_Rank_Structure()



        # self.J278 = pd.read_csv("Inputs/EM_Africa-BARS-Chalco Boffa-Guinea.csv")

        self.J278 = self.restructure.EM_AfricanBArs_Resconstruct()



        # self.Australia = pd.read_csv("Inputs/EM_Region.csv")

        self.Australia = pd.DataFrame(self.dg114["Name3"].copy())

        self.Australia.columns = ["Region"]



        # self.df8 = pd.read_csv("Inputs/EM_LBT Name.csv")

        self.df8 = pd.DataFrame(self.dg114["Name"].copy())

        self.df8.columns = ["name"]



        # self.LBT1 = pd.read_csv("Inputs/EM_LBT PROJECT.csv")

        self.LBT1 = self.restructure.RegionTable



        # self.D1710 = pd.read_csv("Inputs/EM_Supply.csv")

        self.D1710 = self.restructure.EM_Supply_Restructure()
        ...
        self.DDMHardCoded = self.restructure.DDM()



        #####

        # self.dg113 = self.D1710.copy()

        # self.dg113 = self.restructure.EM_Supply_Restructure()

        self.dg113 = pd.read_csv("inputs\EM_Supply.csv")

        self.DDM = pd.read_csv("inputs\EM_DDM.csv")

        # self.DDM = self.DDMHardCoded.copy()

        # self.DDM = self.restructure.DDM()
        ...
        self.DDM2 = self.restructure.total_Demand()



        # self.Drwdwn = pd.read_csv("inputs/EM_Drw Dwn Model.csv")

        self.Drwdwn = self.restructure.DRW_DWN()



        # self.B1817 = pd.read_csv("inputs/EM_1817.csv")

        self.B1817 = self.restructure.CBIX_Actual()



        # self.B2 = pd.read_csv("inputs/EM_BAR.csv")

        self.B2 = self.restructure.Bars_IN_Use_Weiqiao()



        #  self.AMR_supply = pd.read_csv("inputs/AMR_supply.csv")

        self.AMR_supply = self.restructure.AMR_supply()



        # self.LBT = pd.read_csv("inputs/EM_LBT.csv")

        self.LBT = self.restructure.lbt_em()
        ...
        self.BARactual = self.restructure.Bar_For_Act_Supply()
    ...
    def StockDemandandClosingStocks(self):
        ...
        A1 = float(BARactual.iloc[0, 1])

        A = 36.520084519 / A1



        B1 = float(BARactual.iloc[0, 2])

        B = 56.099999178 / B1



        C1 = float(BARactual.iloc[0, 3])
    ...
    def australia(self):
        ...
        for i in range(0, EM1165.shape[1]):
            ...
            # Join115 = pd.merge(D110, EM51, right_index=True, left_index=True)

            # Join115 = pd.merge(D110, EM51, on=["Name"])

            # Join115 = pd.merge(D110, EM51, how="left")
    ...
    def leagueTableBuilder(self):
        ...
        # DB_Data = priceForecastRestructure()

        Supply_with_Countries = self.dg114.copy()  # EM_Supply_with Countries

        African_Bars = self.J278.copy()  # EM African Bars  //integrated no errors

        Supply = self.dg113.copy()  # EM Supply

        # Supply_with_Countries = DB_Data.EM_SupplyCountries_Restructure()

        # African_Bars = DB_Data.EM_AfricanBArs_Resconstruct()

        # Supply = DB_Data.EM_Supply_Restructure()
        ...
        leagueDF.to_csv("Output Files\LeagueTableBuilder).csv", index=False)

        dblist.append(pf_flat.mult_year_single_output(leagueDF, "LeagueTableBuilder"))
