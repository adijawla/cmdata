...
class Causticprice():
    def __init__(self):
        self.cpdata = pd.read_csv("Inputs\\causticpricedata.csv")
        self.ddata = pd.read_csv("Inputs\\domesticpricerawdata1.csv")
        self.caustic_price_data = pd.DataFrame(columns=["month","data","Shandong","Henan","Shanxi"])
        self.caustic_check = pd.DataFrame(columns=["check1","check2","check3"])
        self.domesticpricedata = pd.read_csv("Inputs\\domesticpricedata1.csv")
        self.caustic_check1 = pd.DataFrame(columns=["check1","check2","check3"])
    def liquidcausticshandong(self,indx):
        d = [self.cpdata["Shandong L1"][indx],self.cpdata["Shandong L2"][indx]]
        v = (d[0]+d[1])/2
    ...
    def causticcheck1(self,indx):
        d  = [self.cpdata["Shandong L1"][indx],self.cpdata["Shandong L2"][indx],self.caustic_price_data["Shandong"][indx]]
        v = ((d[0]+d[1])/2)-d[0]
        self.caustic_check.at[indx,"check1"]=v
    
    def causticcheck2(self,indx):
        d = [self.cpdata["Shanxi L1"][indx],self.cpdata["Shanxi L2"][indx],self.caustic_price_data["Shanxi"][indx]]
        v = ((d[0]+d[1])/2)-d[2]
        self.caustic_check.at[indx,"check2"]=v
    
    def causticcheck3(self,indx):
        d = [self.cpdata["Henan L1"][indx],self.cpdata["Henan L2"][indx],self.caustic_price_data["Henan"][indx]]
        v = ((d[0]+d[1])/2)-d[2]
    ...
    def domesticguizhou(self,indx):
        if self.domesticpricedata["Guizhou 5.5 - 6.5"][indx] == 0:
            d = [self.ddata["guizhou5.5"][indx],self.ddata["guizhou6.5"][indx]]
            v = (d[0]+d[1])/2
            self.domesticpricedata.at[indx,"Guizhou 5.5 - 6.5"]=v

    def domesticcheck1(self,indx):
        d = [self.ddata["shanxi4.5"][indx],self.ddata["shanxi5"][indx],self.domesticpricedata["Shanxi 4.5 - 5.0"][indx]]
        v = ((d[0]+d[1])/2)-d[2]
        self.caustic_check1.at[indx,"check 1"]=v

    def domesticcheck2(self,indx):
        d = [self.ddata["henan4"][indx],self.ddata["henan5"][indx],self.domesticpricedata["Henan 4.0 - 5.0"][indx]]
        v = ((d[0]+d[1])/2)-d[2]
        self.caustic_check1.at[indx,"check 2"]=v

    def domesticcheck3(self,indx):
        d = [self.ddata["guizhou5.5"][indx],self.ddata["guizhou6.5"][indx],self.domesticpricedata["Guizhou 5.5 - 6.5"][indx]]
        v = ((d[0]+d[1])/2)-d[2]
...
class Frieght():
    def __init__(self):
        self.frieghtdata = pd.read_csv("Inputs\\frieghtdata.csv")
        self.dailypricedata = pd.read_csv("Inputs\\dailypricedata.csv")
        self.WeeklyChartDataFrame=pd.read_csv("Inputs\\cbixHitoricalDailyPrice.csv")
        self.cmaaxrollingdataFrame=pd.read_csv("Inputs\\cmaaxrollingdata1.csv")
        self.cmaxrollingYearDataFrame=pd.read_csv("Inputs\\cmaxrollingYearData.csv")
        self.cockpit = 43980
        self.frieghtrollingdata = pd.read_csv("Inputs\\frieghtrollingdata.csv")
        self.cbixrollingdata = pd.read_csv("Inputs\\cbixrollingdata.csv")
        self.cmaaxdata1 = pd.read_csv("Inputs\\cmaaxdata1.csv")
        self.cmaaxdata2 = pd.read_csv("Inputs\\cmaaxdata2.csv")
        self.cbixRollingYearData = pd.DataFrame(columns=["day","date","CBIX1","CBIX2"])
        self.cmaaxrollingdata = pd.read_csv("Inputs\\cmaaxrollingdata.csv")
        self.cmaxrollingYearData = pd.DataFrame(columns=["date","northprice","southprice","wacif"])
    ...
    def frieghtrollingcapesize(self,indx):
        d = [self.frieghtdata.loc[self.frieghtdata.weekbeforecbx==self.frieghtrollingdata["week"][indx]]["guinea,capesize"].sum()]
        v = d[0]
        self.frieghtrollingdata.at[indx,"capesize"]=v

    def frieghtrollingpanamax(self,indx):
        d = [self.frieghtdata.loc[self.frieghtdata.weekbeforecbx==self.frieghtrollingdata["week"][indx]]["australia,panamax"].sum()]
        v = d[0]
        self.frieghtrollingdata.at[indx,"panamax"]=v

    def frieghtrollingbci(self,indx):
        d = [self.frieghtdata.loc[self.frieghtdata.weekbeforecbx==self.frieghtrollingdata["week"][indx]]["brazil,capesize"].sum()]
        v = d[0]
    ...
    def cbxrollingdate(self,indx):
        d = [self.dailypricedata.loc[self.dailypricedata.daybeforecbx==self.cbixrollingdata["Day"][indx]]["date"].sum()]
        v = d[0]
        self.cbixrollingdata.at[indx,"Date"]=v
    
    def cbxrollingcbx1(self,indx):
        d = [self.dailypricedata.loc[self.dailypricedata.daybeforecbx==self.cbixrollingdata["Day"][indx]]["cbixusd"].sum()]
        v = d[0]
    ...
    def cmaax1wacif(self,indx):
        ...
        try:
            ...
            check=0
        v = self.cmaaxdata2.loc[self.cmaaxdata2["date"]==check]["wacifrmb"].sum()
    ...
    def weeklychartdata(self,indx):
        d=[pd.DatetimeIndex([self.WeeklyChartDataFrame["date"][indx]]),self.WeeklyChartDataFrame["cbix"][indx]]
        v1 = d[0][0].day_name()
        v2 = str(d[0][0].date())
        v3 = d[1]
...
def main():
    ...
    for frame in caustic_data_frames:
        dblist.append(db_conv.single_year_mult_out(eval(frame), (str(frame).split('.'))[1]))
        eval(frame).to_excel(f"output/{(str(frame).split('.'))[1]}.xlsx",float_format='%.3f')
