...
class AA_Cost_DB():
    ...
    def callall(self):
        ...
        ##here
        self.data2['Ref. No.'] = refs
        self.data2['Digestion technology'] = '?'
        ...
        self.aacost_freight_prices = self.aacost_freight_prices.drop(['aacost_freight_prices','creation_date', 'updation_date'], axis=1)
        ...
        #PDT cost current1
        #self.pdt_cost1input = self.pdt_cost1input.drop(['creation_date', 'updation_date'], axis=1)
        data = pd.DataFrame(columns=[])
        col_new = ['Ref. No.',
                    'Summary Name ',
                    'Chinese Name',
        ...
        for year in list(dict.fromkeys(self.proddatah['year_id'])):
            year_name = dyear[year]
            pro[year_name] = pd.Series(self.proddatah.loc[self.proddatah['year_id']==year,'production'].to_list())
        self.proddatah = pro
