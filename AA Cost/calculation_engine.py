# -*- coding: utf-8 -*-
"""
Created on Fri Sep 25 19:23:05 2020
...
#global imports
import_bx_tonnes_filename = r'inputs/_Import Bx Tonnes & CFR.xlsx'
imported_bx_xtcs_filename = r'inputs/_Imported Bx Characteristics.xlsx'
pdt_cost_current_filename = r'inputs/_PDT cost current.xlsx'
lookup_filename = r'inputs/_Look up tables.xlsx'
refineriesPDT_filename = r'inputs/_Refineries PDT Data.xlsx'
refCapProduction_filename = r'inputs/_Refineries Capacity and Production.xlsx'

Year = str(pd.read_excel(pdt_cost_current_filename, sheet_name='Year').loc[0,'Year'])
calorific_factor = pd.read_excel(lookup_filename, sheet_name='Table 5').loc[0,'Caloriific value conversion factor'] #0.004184     
gassified =pd.read_excel(lookup_filename, sheet_name='Table 3').loc[0,'Energy requirement (GJ/t_aa)']    # 6.5     
natural_gas =  pd.read_excel(lookup_filename, sheet_name='Table 3').loc[1,'Energy requirement (GJ/t_aa)'] #3.2  
...
class PDT_Cost_Current():
    def __init__(self, bxTonnes):
        ...
        self.pdt1_headers = pd.DataFrame(columns=[], index=self.pdt_cost_1.index)
    ...
    def imp_summary_1(self):
        ...
        for row in self.ImpBxSummary.index:
            ...
            #ref valus in columns in table 2 are strings.... 
            col_val = self.func_ref_formater( self.ImpBxSummary.loc[row, 'Ref. No.'])
            try:
                self.ImpBxSummary.at[row,'tonnes'] = self.bxMain.table2[col_val].astype(float).sum()
            except:
    ...
    def imp_summary_2(self):  
        for row in self.ImpBxSummary.index:
            ...
            if self.ImpBxSummary.loc[row,'tonnes']<=0:
                ...
                col_val = self.func_ref_formater( self.ImpBxSummary.loc[row, 'Ref. No.'])
                value = self.bxMain.table3[col_val] * self.bxMain.table2[col_val]
    ...
    def imp_summary_3(self):
        for row in self.ImpBxSummary.index:
            ...
            if self.ImpBxSummary.loc[row,'tonnes']<=0:
                ...
                col_val = self.func_ref_formater( self.ImpBxSummary.loc[row, 'Ref. No.'])
                value = self.bxMain.table2[col_val]  *  self.ImportedBx['Moisture']
                value = value.sum()/self.ImpBxSummary.loc[row,'tonnes']
    ...
    def imp_summary_4(self):
        for row in self.ImpBxSummary.index:
            ...
            if self.ImpBxSummary.loc[row,'tonnes'] <= 0:
                ...
                col_val = self.func_ref_formater( self.ImpBxSummary.loc[row, 'Ref. No.'])
                ans = self.bxMain.table2[col_val] * self.ImportedBx['lt bayer refining.2']
                res = ans.sum()/self.ImpBxSummary.loc[row,'tonnes']
            elif self.ImpBxSummary.loc[row,'Digestion technology'] == 'MT':
                col_val = self.func_ref_formater( self.ImpBxSummary.loc[row, 'Ref. No.'])
                ans = self.bxMain.table2[col_val] * self.ImportedBx['mt bayer refining.2']
                res = ans.sum()/self.ImpBxSummary.loc[row,'tonnes']
            elif self.ImpBxSummary.loc[row,'Digestion technology'] == 'HT':
                col_val = self.func_ref_formater( self.ImpBxSummary.loc[row, 'Ref. No.'])
                ans = self.bxMain.table2[col_val] * self.ImportedBx['ht bayer and sinter.2']
                res = ans.sum()/self.ImpBxSummary.loc[row,'tonnes']
            else:
                res = 'error'
            self.ImpBxSummary.at[row, 'Silica Grade(Reacting)']  = res
    #new function i just added for refs no formatting
    def func_ref_formater(self, ref_no):
        n = ref_no - np.fix(ref_no)
        if n==0:
            res = str(int(ref_no))
        else:
            try:
    ...
    def imp_summary_5(self):
        for row in self.ImpBxSummary.index:
            ...
            if self.ImpBxSummary.loc[row,'tonnes'] <= 0:
                ...
                col_val = self.func_ref_formater( self.ImpBxSummary.loc[row, 'Ref. No.'])
                ans = self.bxMain.table2[col_val] * self.ImportedBx['lt bayer refining.1']
                res = ans.sum()/self.ImpBxSummary.loc[row,'tonnes']
            elif self.ImpBxSummary.loc[row,'Digestion technology'] == 'MT':
                col_val = self.func_ref_formater( self.ImpBxSummary.loc[row, 'Ref. No.'])
                ans = self.bxMain.table2[col_val] * self.ImportedBx['mt bayer refining1']
                res = ans.sum()/self.ImpBxSummary.loc[row,'tonnes']
            elif self.ImpBxSummary.loc[row,'Digestion technology'] == 'HT':
                col_val = self.func_ref_formater( self.ImpBxSummary.loc[row, 'Ref. No.'])
                ans = self.bxMain.table2[col_val] * self.ImportedBx['ht bayer and sinter1']
                res = ans.sum()/self.ImpBxSummary.loc[row,'tonnes']
            else:
                res = 'error'
    ...
    def refPDT_cal_1(self, row):
        ...
        self.refPDT2['Freight - calculated'] = [(self.refPDT2.loc[row,'Option 1 - Freight']  if  float(self.refPDT2.loc[row,'Option']) == 1 else self.refPDT2.loc[row,'Option 2 - Freight'] )for row in range(len(self.refPDT2))]
        self.refPDT2['Freight - Final'] = [ (self.refPDT2.loc[row,'Freight - FS'] if float(self.refPDT2.loc[row,'Freight - FS'])>0 else self.refPDT2.loc[row, 'Freight - calculated']) for row in range(len(self.refPDT2))]
        try:
            res = 0
            if self.refPDT2.loc[row,'Freight - Final'] == '':
                res = 'FALSE'
            else:
                if float(self.refPDT2.loc[row,'Port Handling incl']) == 1:
                    res = self.refPDT2.loc[row,'Freight - Final']
                else:
                    res = float(self.refPDT2.loc[row,'Freight - Final']) + float(port_handling)
    ...
    def refPDT_cal_2(self):
        for row in range(94):
            self.refPDT1.loc[row, 'Bauxite 3 - Transport fee'] = self.refPDT2.loc[row,'Total Transport ']
    ...
    def calc_macroEconomic(self):
        ...
        #headers
        self.pdt1_headers['Ref No']                             = self.pdt_cost_1['Ref. No.']
        self.pdt1_headers['Summary Name']                       = self.pdt_cost_1['Summary Name ']
        self.pdt1_headers['Chinese Name']                       = self.pdt_cost_1['Chinese Name']
        self.pdt1_headers['Sub Name - technology variant']      = self.macro_economic['Sub Name - technology variant']
        self.pdt1_headers['Sub Chinese Name']                   = self.pdt_cost_1['Sub Chinese Name']
        self.pdt1_headers['Province']                           = self.macro_economic['Province']
        self.pdt1_headers['Prov - Category']                    = self.pdt_cost_1['Prov - Category']
        self.pdt1_headers['Owner']                              = self.macro_economic['Owner']
        
    
    def calc_capacityProduction(self):
        year = Year
        self.cap_production['Ref No'] = self.pdt1_headers['Ref No']   
        self.cap_production['Refinery Name'] =self.pdt1_headers['Summary Name']
    ...
    def calc_global(self):
        self.pdt_global['Ref No']= self.pdt1_headers['Ref No']
        self.pdt_global['Refinery Name'] =self.pdt1_headers['Summary Name']
    ...
    def calc_bauxite_1(self):
        self.bauxite_1['Ref No']= self.pdt1_headers['Ref No']
        self.bauxite_1['Refinery Name'] =self.pdt1_headers['Summary Name']
    ...
    def calc_bauxite_2(self):
        self.bauxite_2['Ref No']= self.pdt1_headers['Ref No']
        self.bauxite_2['Refinery Name'] =self.pdt1_headers['Summary Name']
    ...
    def calc_bauxite_3(self):
        self.bauxite_3['Ref No']= self.pdt1_headers['Ref No']
        self.bauxite_3['Refinery Name'] =self.pdt1_headers['Summary Name']
    ...
    def calc_bauxite_sourcing_mix(self):
        self.bauxite_sourcing_mix['Ref No']= self.pdt1_headers['Ref No']
        self.bauxite_sourcing_mix['Refinery Name'] =self.pdt1_headers['Summary Name']
    ...
    def calc_bauxite_final_xtics(self):
        self.bauxite_final_xtics['Ref No']= self.pdt1_headers['Ref No']
        self.bauxite_final_xtics['Refinery Name'] =self.pdt1_headers['Summary Name']
        ...
        self.bauxite_final_xtics['Total Delivered'] = [val4[row] if self.cap_production.loc[row,'Production - sub module']>0 else 0 for row in self.pdt_cost_1.index]
        
    
    def calc_caustic_purchased(self):
        self.caustic_purchase['Ref No']= self.pdt1_headers['Ref No']
        self.caustic_purchase['Refinery Name'] =self.pdt1_headers['Summary Name']
    ...
    def calc_caustic_self_supplied(self):
        self.caustic_selfsupplied['Ref No']= self.pdt1_headers['Ref No']
        self.caustic_selfsupplied['Refinery Name'] =self.pdt1_headers['Summary Name']
        ...
        #new updates here
        self.caustic_selfsupplied['Updated Customisation multiplier'] = self.pdt_cost_1['Updated_Caustic_Customisation_Multiplier']
        
        value1 = self.caustic_selfsupplied['Domestic Purchased']*self.caustic_purchase['Final Price - purchased caustic'] + (1-self.caustic_selfsupplied['Domestic Purchased'])*self.caustic_selfsupplied['Final Price - Self-Supplied caustic']
        self.caustic_selfsupplied['Final Price'] = value1*self.caustic_selfsupplied['Updated Customisation multiplier'] 
        
        
    def calc_energy_sourcing_mix(self):
        self.energy_for_steam['Ref No']= self.pdt1_headers['Ref No']
        self.energy_for_steam['Refinery Name'] =self.pdt1_headers['Summary Name']
    ...
    def calc_energy_lignitous_coal(self):
        ...
        self.energy_for_steam['Calorific value conversion factor'] = calorific_factor
    ...
    def calc_energy_calcing_sourcing_mix(self):
        self.energy_calcining_alumina['Ref No']= self.pdt1_headers['Ref No']
        self.energy_calcining_alumina['Refinery Name'] =self.pdt1_headers['Summary Name']
    ...
    def calc_energy_calcing_anthracite_coal(self):
        ...
        self.energy_calcining_alumina['Calorific value conversion factor'] = calorific_factor
    ...
    def calc_lime_prices_mix(self):
        self.lime['Ref No']= self.pdt1_headers['Ref No']
        self.lime['Refinery Name'] =self.pdt1_headers['Summary Name']
    ...
    def calc_limestone_prices_mix(self):
        self.limestone['Ref No']= self.pdt1_headers['Ref No']
        self.limestone['Refinery Name'] =self.pdt1_headers['Summary Name']
    ...
    def calc_labour_prices_mix(self):
        self.labour['Ref No']= self.pdt1_headers['Ref No']
        self.labour['Refinery Name'] =self.pdt1_headers['Summary Name']
    ...
    def calc_consumables_prices_mix(self):
        self.consumables['Ref No']= self.pdt1_headers['Ref No']
        self.consumables['Refinery Name'] =self.pdt1_headers['Summary Name']
    ...
    def calc_usage_bauxite(self):
        self.usg_bauxite['Ref No']= self.pdt1_headers['Ref No']
        self.usg_bauxite['Refinery Name'] =self.pdt1_headers['Summary Name']
    ...
    def calc_usage_caustic(self):
        self.usg_caustic['Ref No']= self.pdt1_headers['Ref No']
        self.usg_caustic['Refinery Name'] =self.pdt1_headers['Summary Name']
    ...
    def calc_usage_energy(self):
        self.usg_energy['Ref No']= self.pdt1_headers['Ref No']
        self.usg_energy['Refinery Name'] =self.pdt1_headers['Summary Name']
        ...
        self.usg_energy['If Anthracite'] =gassified
        self.usg_energy['If Natural gas/Coal Steam'] = natural_gas
    ...
    def calc_usage_lime_limestone(self):
        self.usg_lime['Ref No']= self.pdt1_headers['Ref No']
        self.usg_lime['Refinery Name'] =self.pdt1_headers['Summary Name']
        ...
        self.usg_limestone['Ref No']= self.pdt1_headers['Ref No']
        self.usg_limestone['Refinery Name'] =self.pdt1_headers['Summary Name']
    ...
    def calc_usage_labour(self):
        self.usg_labour['Ref No']= self.pdt1_headers['Ref No']
        self.usg_labour['Refinery Name'] =self.pdt1_headers['Summary Name']
    ...
    def calc_usage_consumables(self):
        self.usg_consumables['Ref No']= self.pdt1_headers['Ref No']
        self.usg_consumables['Refinery Name'] =self.pdt1_headers['Summary Name']
    ...
    def calc_costs_bauxite(self):
        self.bauxite_cost['Ref No']= self.pdt1_headers['Ref No']
        self.bauxite_cost['Refinery Name'] =self.pdt1_headers['Summary Name']
    ...
    def calc_costs_caustic(self):
        self.caustic_cost['Ref No']= self.pdt1_headers['Ref No']
        self.caustic_cost['Refinery Name'] =self.pdt1_headers['Summary Name']
    ...
    def calc_costs_energy(self):
        self.energy_cost['Ref No']= self.pdt1_headers['Ref No']
        self.energy_cost['Refinery Name'] =self.pdt1_headers['Summary Name']
    ...
    def calc_costs_lime(self):
        self.lime_cost['Ref No']= self.pdt1_headers['Ref No']
        self.lime_cost['Refinery Name'] =self.pdt1_headers['Summary Name']
    ...
    def calc_costs_labour(self):
        self.labour_cost['Ref No']= self.pdt1_headers['Ref No']
        self.labour_cost['Refinery Name'] =self.pdt1_headers['Summary Name']
    ...
    def calc_costs_consumables(self):
        self.consumables_cost['Ref No']= self.pdt1_headers['Ref No']
        self.consumables_cost['Refinery Name'] =self.pdt1_headers['Summary Name']
    ...
    def calc_costs_maintenance(self):
        self.maintenance_cost['Ref No']= self.pdt1_headers['Ref No']
        self.maintenance_cost['Refinery Name'] =self.pdt1_headers['Summary Name']
    ...
    def calc_costs_other(self):
        self.other_cost['Ref No']= self.pdt1_headers['Ref No']
        self.other_cost['Refinery Name'] =self.pdt1_headers['Summary Name']
    ...
    def calc_pdt_summary(self):
        self.PDT1summmary['Ref No']= self.pdt1_headers['Ref No']
        self.PDT1summmary['Refinery Name'] =self.pdt1_headers['Summary Name']
        ...
        self.PDT1summmary['Packaging']           = [self.other_cost['Packaging'][row] if self.cap_production.loc[row,'Production - sub module']>0 else 0 for row in self.pdt_cost_1.index]
    ...
    def calc_pdt_summary2(self):
        self.pdt_1_costSum['Ref No']= self.pdt1_headers['Ref No']
        self.pdt_1_costSum['Refinery Name'] =self.pdt1_headers['Summary Name']
        ...
        self.pdt1_headline['Ref No']= self.pdt1_headers['Ref No']
        self.pdt1_headline['Refinery Name'] =self.pdt1_headers['Summary Name']
...
class PDT_Cost_Current2():
    def __init__(self, bxTonnes, pdt1):
        ...
        self.PDT2_headers = pd.DataFrame(columns=[], index=self.pdt_cost_2.index)
    ...
    def calc_pdt_headers(self):
        ...
        d={pdt_cost.lookup4['Exchange Rate'][x]:pdt_cost.lookup4['USD/Local'][x] for x in pdt_cost.lookup4.index}
        for row in self.pdt_header.index:
            try:
                self.pdt_header.at[row,'Exchange Rate'] = d[self.pdt_header.loc[row,'country']]
            except:
                self.pdt_header.at[row,'Exchange Rate'] = 0
        
    def calc_capacityProduction(self):
        year = Year
        self.cap_production2['Ref No'] = self.pdt_cost_2['Ref. No.']
        self.cap_production2['Refinery name'] = self.pdt_cost_2['Summary Name']
    ...
    def calc_global(self):
        self.pdt_global['Ref No'] = self.pdt_cost_2['Ref. No.']
        self.pdt_global['Refinery name'] = self.pdt_cost_2['Summary Name']
    ...
    def calc_bauxite1(self):
        self.bauxite1['Ref No'] = self.pdt_cost_2['Ref. No.']
        self.bauxite1['Refinery name'] = self.pdt_cost_2['Summary Name']
    ...
    def calc_bauxite2(self):
        self.bauxite2['Ref No'] = self.pdt_cost_2['Ref. No.']
        self.bauxite2['Refinery name'] = self.pdt_cost_2['Summary Name']
    ...
    def calc_bauxite3(self):
        self.bauxite3['Ref No'] = self.pdt_cost_2['Ref. No.']
        self.bauxite3['Refinery name'] = self.pdt_cost_2['Summary Name']
        ...
        self.bauxite3['Total Delivered 1'] = self.bauxite3['Purchase - CFR']*self.pdt_header['Exchange Rate']*(1-self.bauxite3['Moisture'])+self.bauxite3['Total Transport Fee']
        self.bauxite3['Total Delivered 2'] = self.bauxite3['Total Delivered 1']/(1-self.bauxite3['Moisture'])
    
    def calc_bauxite_sourcing_mix(self):
        self.bauxite_sourcing_mix['Ref No'] = self.pdt_cost_2['Ref. No.']
        self.bauxite_sourcing_mix['Refinery name'] = self.pdt_cost_2['Summary Name']
        self.bauxite_sourcing_mix['Domestic Self Supply'] = self.func_1_bauxites('Bauxite Sourcing - Domestic Self Supply')
        self.bauxite_sourcing_mix['Domestic Purchased'] = self.func_1_bauxites('Bauxite Sourcing - Domestic Purchased')
        self.bauxite_sourcing_mix['Import Purchased'] = 1-(self.bauxite_sourcing_mix['Domestic Self Supply']+self.bauxite_sourcing_mix['Domestic Purchased'])
        self.bauxite_sourcing_mix['Total'] = self.bauxite_sourcing_mix['Domestic Self Supply']+self.bauxite_sourcing_mix['Domestic Purchased'] +self.bauxite_sourcing_mix['Import Purchased']
    ...
    def calc_bauxite_final_xtics(self):
        self.bauxite_final_xtics['Ref No'] = self.pdt_cost_2['Ref. No.']
        self.bauxite_final_xtics['Refinery name'] = self.pdt_cost_2['Summary Name']
    ...
    def calc_caustic_price(self):
        self.caustic_price['Ref No'] = self.pdt_cost_2['Ref. No.']
        self.caustic_price['Refinery name'] = self.pdt_cost_2['Summary Name']
    ...
    def calc_energy_raising(self):
        self.energy_for_steam['Ref No'] = self.pdt_cost_2['Ref. No.']
        self.energy_for_steam['Refinery name'] = self.pdt_cost_2['Summary Name']
    ...
    def calc_energy_calcing_alumina(self):
        self.energy_calcining_alumina['Ref No'] = self.pdt_cost_2['Ref. No.']
        self.energy_calcining_alumina['Refinery name'] = self.pdt_cost_2['Summary Name']
    ...
    def calc_lime_limestone(self):
        self.lime['Ref No'] = self.pdt_cost_2['Ref. No.']
        self.lime['Refinery name'] = self.pdt_cost_2['Summary Name']
        self.lime['Lime List Price'] =  self.func_2_lk2('Lime List Price')
        ...
        self.limestone['Ref No'] = self.pdt_cost_2['Ref. No.']
        self.limestone['Refinery name'] = self.pdt_cost_2['Summary Name']
        self.limestone['Limestone List Price']  = self.func_2_lk2('Limestone List Price')
    ...
    def calc_labour(self):
        self.labour['Ref No'] = self.pdt_cost_2['Ref. No.']
        self.labour['Refinery name'] = self.pdt_cost_2['Summary Name']
        self.labour['Labor General List Price']         = self.func_2_lk2('Labor General List Price')
    ...
    def calc_flocculent(self):
        self.consumables['Ref No'] = self.pdt_cost_2['Ref. No.']
        self.consumables['Refinery name'] = self.pdt_cost_2['Summary Name']
        self.consumables['Flocculent List Price'] = self.func_2_lk2('Flocculent List Price')
    ...
    def calc_usagebauxite(self):
        self.usg_bauxite['Ref No'] = self.pdt_cost_2['Ref. No.']
        self.usg_bauxite['Refinery name'] = self.pdt_cost_2['Summary Name']
    ...
    def calc_usagecaustic(self):
        self.usg_caustic['Ref No'] = self.pdt_cost_2['Ref. No.']
        self.usg_caustic['Refinery name'] = self.pdt_cost_2['Summary Name']
        self.usg_caustic['DSP Na:Si'] = self.func_4_lk1('DSP Na:Si')
        self.usg_caustic['Caustic Wash loss'] = self.func_4_lk1('Caustic wash loss')
        value = self.usg_caustic['DSP Na:Si']* self.bauxite_final_xtics['Silica Grade (Reacting)']*self.usg_bauxite['Final bauxite'] + self.usg_caustic['Caustic Wash loss']
        self.usg_caustic['Caustic use'] = value
    ...
    def calc_usageenergy(self):
        self.usg_energy['Ref No'] = self.pdt_cost_2['Ref. No.']
        self.usg_energy['Refinery name'] = self.pdt_cost_2['Summary Name']
        self.usg_energy['Required Steam raising energy'] =self.func_4_lk1('Required Steam raising energy')
    ...
    def calc_usagelimelimestone(self):
        self.usg_lime['Ref No'] = self.pdt_cost_2['Ref. No.']
        self.usg_lime['Refinery name'] = self.pdt_cost_2['Summary Name']
        self.usg_lime['Lime rate'] = self.func_4_lk1('Lime rate')
        self.usg_lime['Custom multiplier'] = self.pdt_cost_2['Usage - Lime Custom mul']
        self.usg_lime['Final Lime rate'] = self.usg_lime['Lime rate']*self.usg_lime['Custom multiplier']
        
        self.usg_limestone['Ref No'] = self.pdt_cost_2['Ref. No.']
        self.usg_limestone['Refinery name'] = self.pdt_cost_2['Summary Name']
        self.usg_limestone['Limestone 1 rate'] = self.func_4_lk1('Limestone rate')
        self.usg_limestone['Limestone 2 rate'] = self.usg_limestone['Limestone 1 rate'] *self.bauxite_final_xtics['Silica Grade (Reacting)']
    ...
    def calc_usagelabour(self):
        self.usg_labour['Ref No'] = self.pdt_cost_2['Ref. No.']
        self.usg_labour['Refinery name'] = self.pdt_cost_2['Summary Name']
    ...
    def calc_usageconsumables(self):
        self.usg_consumables['Ref No'] = self.pdt_cost_2['Ref. No.']
        self.usg_consumables['Refinery name'] = self.pdt_cost_2['Summary Name']
    ...
    def calc_costsbauxite(self):
        self.bauxite_cost['Ref No'] = self.pdt_cost_2['Ref. No.']
        self.bauxite_cost['Refinery name'] = self.pdt_cost_2['Summary Name']
        self.bauxite_cost['bauxite cost'] = self.bauxite_final_xtics['Total Delivered'] * self.usg_bauxite['Final bauxite']
        
    def calc_costscaustic(self):
        self.caustic_cost['Ref No'] = self.pdt_cost_2['Ref. No.']
        self.caustic_cost['Refinery name'] = self.pdt_cost_2['Summary Name']
    ...
    def calc_costsenergy(self):
        self.energy_cost['Ref No'] = self.pdt_cost_2['Ref. No.']
        self.energy_cost['Refinery name'] = self.pdt_cost_2['Summary Name']
        self.energy_cost['Lignitious coal'] = self.usg_energy['Steam energy'] *  self.energy_for_steam['Final Price']
        self.energy_cost['Anthracite/Gas']  = self.usg_energy['Calcining energy'] * self.energy_calcining_alumina['Alumina calcining energy - final price']
        self.energy_cost['Electricity']     = self.usg_energy['Electrical energy - final'] * self.energy_calcining_alumina['Electricity Final Price']
        self.energy_cost['Total energy cost']   = self.energy_cost['Lignitious coal']  + self.energy_cost['Anthracite/Gas'] +  self.energy_cost['Electricity']
    ...
    def calc_costslime(self):
        self.lime_cost['Ref No'] = self.pdt_cost_2['Ref. No.']
        self.lime_cost['Refinery name'] = self.pdt_cost_2['Summary Name']
        self.lime_cost['Lime']              = self.usg_lime['Final Lime rate'] * self.lime['Final Price'] 
        self.lime_cost['Limestone']         = self.usg_limestone['Final Limestone rate'] * self.limestone['Final Price']
        self.lime_cost['Total lime cost']   = self.lime_cost['Lime'] + self.lime_cost['Limestone']

    def calc_costslabour(self):
        self.labour_cost['Ref No'] = self.pdt_cost_2['Ref. No.']
        self.labour_cost['Refinery name'] = self.pdt_cost_2['Summary Name']
        self.labour_cost['Labour'] = self.labour['Final Price - all up costs'] * self.usg_labour['Final Workforce Productivity']
        
    def calc_costsconsumables(self):
        self.consumables_cost['Ref No'] = self.pdt_cost_2['Ref. No.']
        self.consumables_cost['Refinery name'] = self.pdt_cost_2['Summary Name']
        self.consumables_cost['Flocculent']         = self.usg_consumables['Usage'] * self.consumables['Final Price']
    ...
    def calc_costsmaintenance(self):
        self.maintenance_cost['Ref No'] = self.pdt_cost_2['Ref. No.']
        self.maintenance_cost['Refinery name'] = self.pdt_cost_2['Summary Name']
    ...
    def calc_costsother(self):
        self.other_cost['Ref No'] = self.pdt_cost_2['Ref. No.']
        self.other_cost['Refinery name'] = self.pdt_cost_2['Summary Name']
    ...
    def calc_pdtsummary(self):
        self.PDT2summmary['Ref No'] = self.pdt_cost_2['Ref. No.']
        self.PDT2summmary['Refinery name'] = self.pdt_cost_2['Summary Name']
...
class League_Master():
    ...
    def cal_lm_1(self):
        ...
        rank = sorted(self.leaguetab['Cost - Opex'])
        
        value = []
        for row in self.leaguetab.index:
            cnt = self.leaguetab.loc[:row-1,'Cost - Opex'].to_list()
            self.leaguetab.loc[row, 'Rank'] = cnt.count(self.leaguetab.loc[row,'Cost - Opex']) + rank.index(self.leaguetab.loc[row,'Cost - Opex'])+1
...
pdt_cost.ImpBxSummary.to_excel(writer1, sheet_name='Import Bx Summary', encoding='utf-8', index=False)
writer1.save()


writer2 = pd.ExcelWriter('outputs\\PDT Cost Current tab 1.xlsx')
pdt_cost.pdt1_headers.to_excel(writer2, sheet_name='PDT COST 1 Headers',encoding='utf-8', index=False)
