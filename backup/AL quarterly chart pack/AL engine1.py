...
class ALengine():
    ...
    def calc_data_charts_1(self):
        ...
        countries = ['Australia', 'Brazil', 'Fiji', 'Ghana', 'Guinea', 'Guyana', 'India', 'Indonesia', 'Jamaica', 'Saudi Arabia','Malaysia']
        for country in countries:
            for row in range(216, len(self.dataCharts_9)):
                 col_name = country+' '+ 'tonnes'
                 self.dataCharts_9.at[row, col_name] = self.dataInputMontly_bauxite_imports4.loc[row, country]
                 col_name2 = country+' '+ '$/t'
                 self.dataCharts_9.at[row, col_name2] = self.dataInputMontly_bauxite_imports5.loc[row, country]
                 col_name3 = country+' ' + 'BAR'
                 self.dataCharts_9.at[row, col_name3] = self.dataInputMontly_input_bar.loc[row, country]
                 #dataInputMontly_input_bar
        """
        #
    ...
    def Alumina_By_Province_Mt_Aa(self):
        ...
        one =[1]
        two =[1]
        for i in alumina["Date"]:
             if(str(i)== '01-01-16'):
                 one[0] =alumina[alumina['Date']==i].index.values
                 two[0] = Alumina_Province[Alumina_Province["Date"]==i].index.values
        currentIndex = one[0]
        meIndex = two[0]
        for  i in range( (len(Alumina_Province["Date"]) -meIndex )):
    ...
    def Bauxite_Usage(self):
        ...
        for i in me["Date"]:
             if(str(i)== '01-01-17'):
                 one[0] = me[me['Date']==i].index.values
                 two[0] = Bauxite_Usage[Bauxite_Usage["Date"]==i].index.values
        currentIndex = one[0]
        meIndex = two[0]
        for  i in range( (len(Bauxite_Usage["Date"]) -meIndex )):
...
al.minesBxPrdocution_final = set_header_false(al.minesBxPrdocution_final)
...
al.BauxiteSources = set_header_false(al.BauxiteSources)
al.BauxiteSources.to_excel(writer8,sheet_name='Main Table', encoding='utf-8', index=False)
al.BauxiteSourcesSummary = set_header_false(al.BauxiteSourcesSummary)
...
# al.GlobalBx = set_header_false(al.GlobalBx)
...
# al.AAproductions = set_header_false(al.AAproductions)
al.AAproductions.to_excel(writer10,sheet_name='Main Table', encoding='utf-8', index=False)
writer10.save()
...
dblist = [
    ...
    al_flat.single_year_mult_out(al.Alumina_Province, "Alumina Province"),
    al_flat.single_year_mult_out(al.Bauxite_Usage_Sheet, "Bauxite usage sheet"),
    ...
    # empty tables
    al_flat.single_year_mult_out(al.dataCharts_11, "dataCharts_11"),
    al_flat.single_year_mult_out(al.dataCharts_12, "dataCharts_12"),
    al_flat.single_year_mult_out(al.dataCharts_13, "dataCharts_13"),
