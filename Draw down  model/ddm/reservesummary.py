...
class summary():
    def __init__(self,cbixswitch):
        self.cbixswitch = cbixswitch
        db_conv = Flatdbconverter("Draw Down Model")
        lsnap = x.input_db[15]['shapshot_id'][0]
        self.em = read_output_database(lsnap, ["Max Economic Tonnes"])
        self.max_economic_tonnes = db_conv.reverse(self.em, "Economic overlay", ["Max Economic Tonnes"])
        self.max_economic_tonnes = self.max_economic_tonnes['Economic overlay']['Max Economic Tonnes']
        self.eo = pd.DataFrame(self.max_economic_tonnes)
        '''
        o = x.data('country_reserve')
        cc = self.eo.shape[0]
        for i in range(o.shape[0]):
            self.eo.at[cc+i,"County"] = o.at[i,"county"]
            self.eo.at[cc+i,"Province"] = o.at[i,"province"]
            self.eo.at[cc+i,'Max Economic Tonnes - blending in country'] = o.at[i,"working_stock_prior_to_aa_and_as_de_rating000t"]
        print(lsnap)
        print(self.max_economic_tonnes)
        self.eo.sort_values(by=['County','Province'],ascending=[True, True],inplace=True)
        self.eo = self.eo.reset_index()'''
    ...
    def calc0(self,i):
        summary_timer.start()
        v = 1/(1-self.reservedb['refractory_allocation'][i]) if self.reservedb['refractory_allocation'][i] > 0 else 1
    ...
    def calc2(self,i):#refractory_allocation
        summary_timer.start()
        v = self.reservedb['Inventory'][i]*self.reservedb['refractory_allocation'][i]
    ...
    def calc8(self,i):#working_stock_prior_to_AA_and_AS_de_rating000t
        ...
        if not pd.isnull(self.reservedb["working_stock_prior_to_aa_and_as_de_rating000t"][i]):
            self.db.at[i,"working_stock_prior_to_AA_and_AS_de_rating000t"] = self.reservedb["working_stock_prior_to_aa_and_as_de_rating000t"][i]
        summary_timer.stop()
    def calc9(self,i):#de_rated_Al2O3
        ...
        if not pd.isnull(self.reservedb["de_rated_al2o3"][i]) :
            self.db.at[i,"de_rated_Al2O3"] = self.reservedb["de_rated_al2o3"][i]
        summary_timer.stop()
    ...
    def calc11(self,i):#de_rated_SiO2
        ...
        if not pd.isnull(self.reservedb["de_rated_sio2"][i]):
            self.db.at[i,"de_rated_SiO2"] = self.reservedb["de_rated_sio2"][i]/100
        summary_timer.stop()
    ...
    def calc15(self,i):#SGA_depletion_end_2009
        summary_timer.start()
        x = self.staticdb.iloc[i,4:((self.staticdb.shape[1]-4)//8)+4].reset_index(drop=True)
        y = self.bfdb["estimated_reserves_consumed_per_year"].reset_index(drop=True)
        z = x*y
        z = z.sum()
        v = min(1,z)/(1-self.reservedb['refractory_allocation'][i])
    ...
    def calc30(self,i):#SiO2_end_2009_3
        ...
        v = ((v1*v2)-(((v1-0.03)+((v1-0.03)+v0*0.06))/2)*v0*v2)/((1-v0)*v2) if 1-v0 > 0 and v2 > 0 else 0
    ...
    def calc33(self,i):#check_sum
        summary_timer.start()
        v = self.staticdb.iloc[i,4:((self.staticdb.shape[1]-4)//8)+4].sum() + self.reservedb['refractory_allocation'][i]
    ...
    def calc47(self):
        ...
        v = self.db["working_stock_prior_to_AA_and_AS_de_rating000t"].iloc[:self.db.shape[0]].sum()
    ...
    def calc49(self):
        ...
        v = self.db["de_rated_Al2O3"].iloc[:self.db.shape[0]]*self.db["working_stock_prior_to_AA_and_AS_de_rating000t"].iloc[:self.db.shape[0]]
    ...
    def calc50(self):
        ...
        v = self.db["de_rated_SiO2"].iloc[:self.db.shape[0]]*self.db["working_stock_prior_to_AA_and_AS_de_rating000t"].iloc[:self.db.shape[0]]
        v = v.sum()/self.totaldb["working_stock_prior_to_AA_and_AS_de_rating000t"][0]
    ...
    def calc55(self):
        ...
        v = self.db["SGA_depletion_end_2009"].iloc[:self.db.shape[0]]*self.db["working_stock_prior_to_AA_and_AS_de_rating000t"].iloc[:self.db.shape[0]]
        v = v.sum()/self.totaldb["working_stock_prior_to_AA_and_AS_de_rating000t"][0]
    ...
    def provincialtotalcalc(self,i):
        ...
        temp1 = 0 if v9 == 0 else v11.sum()/v9
        v11 = 0 if temp1 == 0 else v10/temp1
    ...
    def calcnewalloccalc(self,i,ref):
        summary_timer.start()
        v0 = self.db["working_stock_prior_to_AA_and_AS_de_rating000t"][i]*(1-self.bfdb.loc[self.bfdb.Name==ref]["estimated_reserves_consumed_per_year"].sum())*self.staticdb[ref+" Allocation"][i]/(1-self.reservedb['refractory_allocation'][i])
    ...
    def calcall1(self):
        ...
        if self.cbixswitch == 'cbix':
            self.db["working_stock_prior_to_AA_and_AS_de_rating000t"] = self.eo['Max Economic Tonnes - blending in country']
    ...
    def calcall2(self,proddata10):
        ...
        summary.calc72(self)
        for i in range(self.l):
            if self.reservedb["Include Inventory in Allocations Further Processed?"][i] == "NO" and self.db["County"][i] == 'Not Applicable':
