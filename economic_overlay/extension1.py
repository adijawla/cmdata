...
class inputprocess():
    def __init__(self, ids, inputs, id_name):
        ...
        database = 'input_db'
        db2 = 'outputdb_updated'
        username = 'letmetry'
        password = 'T@lst0y50'
        driver = '{ODBC Driver 17 for SQL Server}'
        conn = pyodbc.connect('DRIVER='+driver+';PORT=1433;SERVER='+server +
                              ';PORT=1443;DATABASE='+database+';UID='+username+';PWD=' + password)
        self.conn2 = pyodbc.connect('DRIVER='+driver+';PORT=1433;SERVER='+server +
                              ';PORT=1443;DATABASE='+db2+';UID='+username+';PWD=' + password)
        self.conn = conn
        for i in ids:
            self.id_db.append(pd.read_sql_query(
                "SELECT * FROM [dbo].["+str(i)+"]", conn))
            self.id_db[-1].name = i
        
            try:
                self.id_db[-1].drop(
                    columns=["creation_date", "updation_date"], inplace=True)
            except:
                pass
            try:
                self.id_db[-1].drop(
                    columns=["creation_date_x", "updation_date_x"], inplace=True)
            except:
                pass
            try:
                self.id_db[-1].drop(
                    columns=["creation_date_y", "updation_date_y"], inplace=True)
            except:
                pass
        for i in inputs:
            self.input_db.append(pd.read_sql_query(
                "SELECT * FROM [dbo].["+str(i)+"]", conn))
            self.input_db[-1].name = i
            
            try:
                self.input_db[-1].drop(
                    columns=["creation_date", "updation_date"], inplace=True)
            except:
                pass
            try:
            ...
            try:
                ...
                pass
            for j in self.input_db[-1].columns:
                try:
                    self.input_db[-1][j] = self.input_db[-1][j].astype(float)
        ...
        for i in range(len(self.input_db)):
            for j in self.input_db[i].columns:
                if j in self.id_name:
                    self.input_db[i] = pd.merge(
                        self.id_db[self.id_name.index(j)], self.input_db[i], on=j)
    ...
    def pivot(self, inputname, col, val, indx=[]):
        ...
        self.input_db[i] = pd.pivot_table(
            self.input_db[i], index=indx, values=val, columns=col, aggfunc='sum')
