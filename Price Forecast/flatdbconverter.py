...
class upload():
    def __init__(self,snapshot_output_data):
        ...
        query = "INSERT INTO snapshot_table (shapshot_id,model_id,snapshot_run_date,snapshot_run_time,create_user_id) VALUES ("+str(v.id)+","+str(snapshot_output_data['model_id'][0])+",{d'"+str(date)+"'},{t'"+str(time1)+"'},0)"
...
class Flatdbconverter():
    ...
    def get_snapshot_id(self, snapshot):
            lookup = {**self.snapshot_ids}
            max_id =  max(lookup.values()) if len(lookup.values()) > 0 else 0
            if snapshot not in lookup:
                max_id += 1
                lookup[snapshot] = max_id
            self.snapshot_ids = lookup
    ...
    def multi_year_multi_out(self, filepath, output_set_name, label="Year", idx_of_index=[], idx_of_values=[], model=None, path=False, col_params=[], make_multi=None):
        ...
        # print(len(idx_of_index) > 0 and len(idx_of_values) > 0)
        if len(idx_of_index) > 0 and len(idx_of_values) > 0 :
        ...
        # dump config
        setups[model][output_set_name]["config"]["field"] = field
        dump_configs(setups)
    ...
    def multi_index_multi_out(self, filepath, output_set_name, idx_of_index, idx_of_values,val_label="Year", model=None, path=False):
        if model is None:
            model = self.model
        if path:
            data = pd.read_csv(filepath).dropna(how='all')
        else:
            data = filepath.copy().dropna(how='all').dropna(how='all', axis=1)
        output = pd.DataFrame(columns=[])
        rows = data.shape[0]
        columns = data.columns
        indexes = get_val_from_idx(idx_of_index, columns)
        lookup_values = get_val_from_idx(idx_of_values, columns)
        data.set_index(indexes, inplace=True)
        row_ind_values = [a if isinstance(a, (tuple, list, set)) else [a] for a in data.index]
        output_label = np.array([[[*indexes, val_label, *data.loc[tuple(ind), val_head].values] for val_head in lookup_values] for ind in row_ind_values])
        output_label = print(output_label)
